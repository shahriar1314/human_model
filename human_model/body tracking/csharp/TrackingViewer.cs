...
public class TrackingViewer {
    ...
    public static void render_2D(
        ref OpenCvSharp.Mat left_display,
        sl.float2 img_scale,
        ref sl.Bodies bodies,
        bool showOnlyOK,
                ...
                if (body_format == BODY_FORMAT.BODY_38) {
                    ...
                    foreach (var kp in bod.keypoints2D) {
                        ...
                        if (roi_render.Contains(cv_kp)) {
                            ...
                            // Cv2.PutText(left_display, Array.IndexOf(bod.keypoints2D,kp).ToString(), cv_kp, HersheyFonts.HersheyPlain, 1,
                            // base_color);
                        }
                    }
                }
                ...
                if (render_mask) {
                    ...
                    if (mask.IsInit()) {
                        ...
                        OpenCvSharp.Mat tmp_mask
                            = new OpenCvSharp.Mat(mask.GetHeight(), mask.GetWidth(), OpenCvSharp.MatType.CV_8UC1, m.GetPtr());
                        if (!tmp_mask.Empty()) {
