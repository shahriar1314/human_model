...
# Optional viewers (keep your original behavior)
    ...
    def frames(self):
        ...
        Yields dicts:
          {
            ...
            "bodies": list of { "id": int, "confidence": int, "kp3d": (18,3) float32, "tracking_state": ... }
          }
        """
        if self.enable_view:
        ...
        while availability_fn():
            if self.zed.grab() <= sl.ERROR_CODE.SUCCESS:
                ...
                image_bgr = self.image.get_data()  # BGRA actually; OpenCV still displays ok

                out_bodies = []
                for body in self.bodies.body_list:
                    kps3d = np.array(body.keypoint, dtype=np.float32)  # (18,3) if BODY_18
                    if kps3d.shape[0] >= 18:
                        kps3d = kps3d[:18, :]
                    else:
                        # pad if needed (shouldn't happen in BODY_18)
