...
[
\theta_t \in \mathbb{R}^{11}
]

These 11 variables control upper body articulation and lower body rigid locomotion:

* Left shoulder: yaw, pitch, roll (3)
* Right shoulder: yaw, pitch, roll (3)
* Left elbow: flexion (1)
* Right elbow: flexion (1)
* Lower body (rigid): x-translation, z-translation, roll-rotation (3)

The lower body is treated as a single rigid part on the ground plane (y=0), simplifying lower limb kinematics while preserving upper body expressiveness.
...
* Pelvis positioned at (lb_x, 0, lb_z) with ground-plane constraint (y=0)
* Pelvis rotated by lower body roll (lb_roll)
* Neck above pelvis
* Shoulders placed laterally using fixed offsets
* Arms generated by shoulder rotation + elbow hinge
* Hips positioned relative to pelvis, both legs move identically (rigid coupling)
* Legs follow pelvis orientation (no knee/ankle flexion; kinematically determined)
...
Encourages left/right similarity for the upper body:

* shoulder yaw mirrored (L ≈ −R)
* shoulder pitch same
* shoulder roll mirrored
* elbow flexions similar

Lower body has no symmetry constraint because it is treated as a single rigid unit controlling the overall locomotion, not individual limb articulation.

This reduces unrealistic asymmetric arm solutions.
...
* State dimension is now 11 DOF (reduced from 16), making Hessian solves ~31% faster
